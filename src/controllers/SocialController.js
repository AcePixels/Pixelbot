...
module.exports = class SocialController extends Controller {
  ...
  validateProfile (entity) {
    const { error, value } = Joi.object().keys({
      bio: Joi.string().max(this.PERSONAL_TEXT_LIMIT).truncate().empty(''),
  ...
  async setbio (_user, bio) {
    if (bio.length > this.PERSONAL_TEXT_LIMIT) throw new Error('TEXT_LENGTH')
    await this._users.update(_user, { bio })
  }
  ...
  retrieveProfile (_user, projection = 'money rep bio favColor') {
